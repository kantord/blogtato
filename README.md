...
- Ingest RSS or Atom generated by your own scripts, cron jobs, or CI
...
For pages that do not publish RSS or Atom, you can generate a feed yourself
with a small local scraper and pipe it into `blogtato`:

```bash
python /path/to/rss-scrapers/hn-updates.py | blog feed ingest hn-updates
```
...
Because ingestion is just "run something, pipe RSS/Atom into blogtato", it
composes with anything:

- A cron job or systemd timer that scrapes a page and ingests it periodically
- A GitHub Action in your blogtato repo, so `blog sync` on any machine already
