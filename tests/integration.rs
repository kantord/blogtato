...
impl TestContext {
    ...
    fn run_with_stdin(&self, args: &[&str], stdin: &str) -> assert_cmd::assert::Assert {
        #[allow(deprecated)]
  ...
  <channel>
    ...
    <item>
      ...
      <link>{link}</link>
        ...
        date = recent_rss_date(1)
    )
}

#[test]
fn test_ingest_creates_feed_and_posts() {
    let ctx = TestContext::new();
    let xml = ingest_rss(
        "HN updates",
        "https://hnrss.org/",
        ...
        "Ingested Post",
        "ingested-post-1",
        "https://example.com/ingested-post",
    );

    ctx.run_with_stdin(&["feed", "ingest", "hn-updates"], &xml)
        .success();

    let posts = ctx.read_posts();
    assert_eq!(posts.len(), 1);
    assert_eq!(posts[0]["title"].as_str().unwrap(), "Ingested Post");
    assert_eq!(
    ...
    let feeds = ctx.read_feeds();
    assert_eq!(feeds.len(), 1);
    assert_eq!(feeds[0]["url"].as_str().unwrap(), "stdin:hn-updates");
...
fn test_ingest_twice_updates_same_feed_without_duplicating() {
    ...
    ctx.run_with_stdin(&["feed", "ingest", "tg-channel"], &first)
        .success();

    let second = ingest_rss(
        "TG channel",
        "https://t.me/s/channel_name",
        "Generated with limit 20",
        "Telegram Post 2",
        "tg-2",
        "https://t.me/s/channel_name/2",
    );
    ctx.run_with_stdin(&["feed", "ingest", "tg-channel"], &second)
        .success();

    let feeds = ctx.read_feeds();
    assert_eq!(feeds.len(), 1);
    assert_eq!(feeds[0]["url"].as_str().unwrap(), "stdin:tg-channel");

    let posts = ctx.read_posts();
    let titles: Vec<_> = posts.iter().map(|p| p["title"].as_str().unwrap()).collect();
    assert!(titles.contains(&"Telegram Post 1"));
    assert!(titles.contains(&"Telegram Post 2"));
}

#[test]
fn test_ingest_then_sync_does_not_touch_stdin_feed() {
    let ctx = TestContext::new();
    let xml = ingest_rss(
        "HN updates",
        "https://hnrss.org/",
...
#[test]
fn test_ingest_applies_configured_ingest_filter() {
    let ctx = TestContext::new();
    ctx.run(&[
        "config",
        "set",
        "ingest_filter",
        r#"[.[] | select(.title | startswith("[Sponsored]") | not)]"#,
    ])
    .success();
...
<rss version="2.0">
  <channel>
    ...
    <item>
      <title>[Sponsored] Buy stuff</title>
      <pubDate>{date}</pubDate>
      <guid>ingested-sponsored</guid>
      <link>https://example.com/sponsored</link>
    </item>
    <item>
      <title>Real Post</title>
      <pubDate>{date}</pubDate>
      <guid>ingested-real</guid>
    ...
    assert_eq!(
        titles,
        vec!["Real Post"],
        "sponsored post should have been filtered out by ingest_filter, got: {titles:?}"
    );
}
...
{"id":"3","title":"Post C","date":"2024-01-14T00:00:00Z","feed":"Alice"}"#;
    ...
    let output = ctx
        .run(&["--compact", "show", "/d", "2020-01-01.."])
        .success();
    let stdout = output.stdout_str();

    // In compact mode, there should be no blank lines between group header and items,
    // nor between the last item of a group and the next group header
    ...
    for (i, line) in lines.iter().enumerate() {
        if line.contains("===") {
            ...
            if i > 0 {
                assert!(
                    !lines[i - 1].trim().is_empty(),
                    "compact mode should not have blank line before group header at line {i}: {line}"
                );
            }
        }
    }
    ...
    for (i, line) in lines.iter().enumerate() {
        if line.contains("===") {
            ...
            if i > 0 {
                assert!(
                    !lines[i - 1].trim().is_empty(),
                    "compact config should not have blank line before group header at line {i}: {line}"
                );
            }
        }
    }
